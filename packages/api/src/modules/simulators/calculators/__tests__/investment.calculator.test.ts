...
describe('calculateInvestment() — lump sum, no contributions', () => {
  ...
  it('totalReturns = finalValue - initialAmount', () => {
    ...
    expect(
      Math.abs(result.totalReturns - (result.finalValue - result.totalContributed)),
...
describe('calculateInvestment() — scenarios', () => {
  ...
  it('conservative scenario has lower finalValue than base', () => {
    const result = calculateInvestment(BASE_INPUTS);
    expect(result.scenarios!.conservative.finalValue).toBeLessThan(
      result.scenarios!.base.finalValue,
    );
  });

  it('optimistic scenario has higher finalValue than base', () => {
    const result = calculateInvestment(BASE_INPUTS);
    expect(result.scenarios!.optimistic.finalValue).toBeGreaterThan(
      result.scenarios!.base.finalValue,
    );
  });
  ...
  it('conservative uses annualReturn - 2%', () => {
    ...
    const conservative = calculateInvestment(
      { ...BASE_INPUTS, annualReturn: BASE_INPUTS.annualReturn - 2 },
      false,
    );
    expect(
      Math.abs(
        calculateInvestment(BASE_INPUTS).scenarios!.conservative.finalValue -
          conservative.finalValue,
      ),
    ).toBeLessThanOrEqual(1);
  });

  it('optimistic uses annualReturn + 2%', () => {
    const optimistic = calculateInvestment(
      { ...BASE_INPUTS, annualReturn: BASE_INPUTS.annualReturn + 2 },
      false,
    );
    expect(
