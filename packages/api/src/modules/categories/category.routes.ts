...
function handleCategoryError(error: unknown, reply: FastifyReply): FastifyReply {
...
export async function registerCategoryRoutes(fastify: FastifyInstance): Promise<void> {
  // GET /categories
  ...
  // PATCH /categories/:id
  fastify.patch('/categories/:id', { preHandler: requireAuth }, async (request, reply) => {
    const { userId } = request.user;
  ...
  fastify.delete('/categories/:id', { preHandler: requireAuth }, async (request, reply) => {
    const { userId } = request.user;
    const { id } = request.params as { id: string };
