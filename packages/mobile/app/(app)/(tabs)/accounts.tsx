...
const SwipeableAccountRow: React.FC<SwipeableAccountRowProps> = memo(
  ({ account, colors, rowBg, onPress, onEdit, onAdjust, onArchive }) => {
      ...
      <View style={{ backgroundColor: colors.expense, marginBottom: 1 }}>
        ...
        >
          <View style={[rowStyles.row, { backgroundColor: rowBg }]}>
            {/* Tappable area: icon + info + balance */}
            <TouchableOpacity
              style={rowStyles.rowContent}
              onPress={() => {
                if (offset.current === 0) onPress(account);
              }}
              activeOpacity={0.7}
            >
          ...
          <View style={styles.listWrap}>
            ...
            {accounts.map((account) => (
              <SwipeableAccountRow
                ...
                onPress={(a) =>
                  router.push({
                    pathname: '/(app)/account-detail' as any,
                    params: { id: a._id },
                  })
                }
                onEdit={handleEdit}
